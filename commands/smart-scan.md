...
Generated by /security-kit smart-scan.
...
Options:
  ...
  3. Execute with raptor (requires /security-kit install-raptor)
...
- **No skills installed yet** → smart-scan still works (fingerprint is useful in its own right), but ask the user to run `/security-kit install` or offer to install the variant best-matched to the fingerprint (e.g., `sec-review` for most projects, `threat-modeling` only if the user explicitly asks for a threat model).
